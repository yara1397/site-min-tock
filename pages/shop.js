...
const CATS = [
  {id:'all',label:'ALL'},{id:'template',label:'TEMPLATE'},{id:'animation',label:'MOTION'},
  {id:'icon',label:'ICONS'},{id:'font',label:'FONTS'},{id:'3d',label:'3D'},{id:'texture',label:'TEXTURE'},
]
const TAG_C = { BESTSELLER:'var(--gold)', NEW:'var(--neon)', HOT:'var(--hot)', PREMIUM:'var(--sky)', POPULAR:'#FF6B00', EXCLUSIVE:'#CC88FF' }
  ...
  return (
    <div className="page-wrap">
      ...
      <div style={{ background:'var(--ink2)', borderBottom:'1px solid var(--rim)', padding:'20px 28px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
        <div>
          <div style={{ fontFamily:'var(--display)', fontSize:24, fontWeight:800 }}>🛍️ فروشگاه</div>
          <div style={{ fontFamily:'var(--mono)', fontSize:11, color:'var(--muted)', marginTop:3 }}>{list.length} محصول</div>
        </div>
        <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'center' }}>
          <div style={{ display:'flex', gap:6 }}>
            {CATS.map(c => (
              <button key={c.id} onClick={()=>setCat(c.id)} style={{
                padding:'6px 14px', borderRadius:50, fontSize:11, letterSpacing:1, fontWeight:600,
                fontFamily:'var(--mono)',
                border:`1px solid ${cat===c.id?'rgba(0,255,180,0.4)':'var(--rim)'}`,
                background: cat===c.id?'rgba(0,255,180,0.1)':'transparent',
                color: cat===c.id?'var(--neon)':'var(--muted)', transition:'all 0.2s',
              }}>{c.label}</button>
            ))}
          </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(268px,1fr))', gap:20, padding:'28px' }}>
        {list.map(p => {
          ...
          return (
            <div key={p.id} className="card" style={{ overflow:'hidden' }}>
              <div style={{ height:180, display:'flex', alignItems:'center', justifyContent:'center', fontSize:68, background:'linear-gradient(135deg,var(--ink3),var(--ink2))', position:'relative' }}>
                {p.emoji}
                {p.tag && <span style={{ position:'absolute', top:10, right:10, fontSize:10, fontFamily:'var(--mono)', fontWeight:600, letterSpacing:1, padding:'3px 9px', borderRadius:4, border:`1px solid ${TAG_C[p.tag]}33`, background:`${TAG_C[p.tag]}18`, color:TAG_C[p.tag] }}>{p.tag}</span>}
              </div>
              <div style={{ padding:20 }}>
                <div style={{ fontFamily:'var(--mono)', fontSize:10, color:'var(--sky)', letterSpacing:2, marginBottom:6 }}>{p.cat.toUpperCase()}</div>
                <div style={{ fontSize:16, fontWeight:700, marginBottom:7, lineHeight:1.3 }}>{p.name}</div>
                <div style={{ fontSize:13, color:'var(--dim)', lineHeight:1.5, marginBottom:14, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{p.desc}</div>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
                  <span style={{ color:'var(--gold)', fontSize:13 }}>⭐ {p.rating} <span style={{ color:'var(--muted)' }}>({p.reviews})</span></span>
                  <span style={{ fontFamily:'var(--mono)', fontSize:11, color:'var(--muted)' }}>{p.sales.toLocaleString('fa-IR')} فروش</span>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
                  <span style={{ fontFamily:'var(--display)', fontSize:19, color:'var(--neon)', fontWeight:800 }}>{p.price.toLocaleString('fa-IR')} ت</span>
                ...
                <button onClick={() => handleAdd(p)} style={{
                  ...
                  border: isAdded ? '1px solid var(--neon)' : '1px solid rgba(0,255,180,0.25)',
                  background: isAdded ? 'rgba(0,255,180,0.15)' : 'rgba(0,255,180,0.08)',
                  color:'var(--neon)',
                }}>
