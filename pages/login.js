...
import { doSignIn, getProfile } from '../lib/supabase'
...
function mapErr(msg = '') {
  ...
  if (msg.includes('network') || msg.includes('fetch'))
    return 'خطای اتصال — اینترنت خود را بررسی کنید'
...
export default function LoginPage({ user }) {
  ...
  // اگه قبلاً لاگین بود redirect کن
  useEffect(() => {
  ...
  const submit = async e => {
    ...
    try {
      ...
      if (data?.user) {
        ...
        // بعد از ورود، role کاربر رو چک کن
        try {
          const { data: profileData } = await getProfile(data.user.id)
          setTimeout(() => {
            if (profileData?.role === 'admin') {
              // اگه ادمین بود و از /admin اومده بود برگرده به ادمین
              window.location.href = from.includes('admin') ? '/admin' : from
            } else {
              // کاربر عادی — نمی‌تونه بره /admin
              const safeDest = from.includes('admin') ? '/' : from
              window.location.href = safeDest
            }
          }, 600)
        } catch {
          setTimeout(() => { window.location.href = from.includes('admin') ? '/' : from }, 600)
        }
      }
